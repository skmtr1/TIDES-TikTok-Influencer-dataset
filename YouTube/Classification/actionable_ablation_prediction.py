...
df_videos = pd.read_csv("../Dataset/youtube_videos.csv")
df_infl = pd.read_csv("../Dataset/youtube_influencers.csv")
...
col_to_keep = ['authorId','createTime','video_hasText','duration','height','width','heightXwidth']
...
col_to_keep_infl = ['id','continent','actual_tier','has_signature','has_bioLink','tot_videos']
...
features_to_agg = ['height', 'width','heightXwidth', 'duration']
...
algorithms = ["DT", "XGB", "RF", "LR"] #["XGB", "RF", "LR","DT","NB","SVM","DNN","KNN"] #['RF',"XGB","LR","DT","NB","SVM","MLP","KNN"]
k_bests = ['all']
...
column_combinations = [general_features, all] #[ video_features,general_features, audio_features, text_features]

# column_combinations = list(powerset(column_combinations))
column_names_combinations = ["('general',)", "('video', 'general', 'audio', 'text')"]
        ...
        for i, col_ablation in enumerate(column_combinations):
            ...
            for k_best in k_bests:
                ...
                for train_index,test_index in sss.split(X, y):
                    ...
                    df_results.to_csv("Actionable_ablation_results.csv", index = False)
