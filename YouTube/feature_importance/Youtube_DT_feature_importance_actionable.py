...
def select_best_model(df_ab, tier, feature, iter):
    ...
    df_t = (df_ab[(df_ab['columns']==feature) & (df_ab['iter']==iter) & (df_ab['couples']==couple) & (df_ab['classifier']=='DT')]).reset_index(drop=True)
...
algorithms = ["DT"] #["XGB", "RF", "LR","DT","NB","SVM","DNN","KNN"] #['RF',"XGB","LR","DT","NB","SVM","MLP","KNN"]
k_bests = ['all']
...
df_param = pd.read_csv('Youtube_actionable_for_FI.csv')
            ...
            for k_best in k_bests:
                ...
                for train_index,test_index in sss.split(X, y):
                    ...
                    try:
                        ...
                        df_feature_imp.to_csv(f"Youtube_DT_actionable_{file_save_name[i]}_feature_importance.csv", index=False)
                    ...
                    df_results.to_csv("Youtube_DT_actionable_fi_results_cross_val.csv", index = False)
