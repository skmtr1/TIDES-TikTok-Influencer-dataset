...
class FeatureBinsCalculator(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y = None):
        ...
        for x in ["likes","shares","comments","views", "duration"]:
            for i in range(0,6):
                X_[f"{x}_bin_{i}"] = X_.apply(lambda row: row[f'{x}_distr'][i], axis = 1)
        
        col_to_drop = ['likes_distr', 'shares_distr','comments_distr','views_distr','duration_distr'
              ]
    ...
    for alg in algorithms:
        for k_best in k_bests:
            ...
            for train_index,test_index in sss.split(X, y):
                ...
                # define search
                search = GridSearchCV(pipe, param_grid, scoring='f1_macro', cv=cv_inner, refit=True, n_jobs = -1)
                ...
                try:
                    ...
                    df_feature_imp.to_csv("feature_importance.csv", index=False)
                ...
                df_results.to_csv("results_cross_val.csv", index = False)
