...
class FeatureBinsCalculator(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y = None):
        ...
        for x in ["duration"]:#["likes","shares","comments","views", "duration"]:
            for i in range(0,6):
                X_[f"{x}_bin_{i}"] = X_.apply(lambda row: row[f'{x}_distr'][i], axis = 1)
        
        col_to_drop = ['duration_distr']#['likes_distr', 'shares_distr','comments_distr','views_distr','duration_distr']
...
features_to_agg = ['height', 'width','heightXwidth', 'duration', 'definition',
       'n_mentions', 'n_hashtags', 'n_effects']
...
#pandas results
df_results = pd.DataFrame(columns = ['couples', 'classifier', 'iter', 'precision', 'recall','f1','accuracy', 'params', 'k_best'])

df_feature_imp = None


couples =  [('nano','micro'), ('micro','mid'), ('mid','macro'), ('macro','mega')]
algorithms = ["XGB", "RF", "LR","DT","NB","SVM","DNN","KNN"] #['RF',"XGB","LR","DT","NB","SVM","MLP","KNN"]
k_bests = [10,20,50,'all']
...
df_infl.to_csv("Actionable_DF_INFL.csv", index=False)
...
for c in couples:
    for alg in algorithms:
        for k_best in k_bests:
            ...
            print(f"====== ANALYZING {c} - {alg} - {k_best} ======")
            ...
            for train_index,test_index in sss.split(X, y):
                ...
                pipe = Pipeline(steps=[ 
                    ("distr_calc", FeatureBinsCalculator()), 
                ...
                # define search
                search = GridSearchCV(pipe, param_grid, scoring='f1_macro', cv=cv_inner, refit=True, n_jobs = 12)
                ...
                if df_feature_imp is None:
                    df_feature_imp = pd.DataFrame(columns = ["couple","classifier","kbest","iteration"] + best_model['distr_calc'].features_names)
                ...
                try:
                    ...
                    df_feature_imp.to_csv("actionable_feature_importance.csv", index=False)
                ...
                df_results.to_csv("actionable_results_cross_val.csv", index = False)
